...
import { MapPin, Search, CheckCircle2, Star, Sparkles, Globe, Menu } from 'lucide-react';

export default function IntermissivistasScreen({ onNavigate }: { onNavigate: (screen: string) => void }) {
  const { profiles, currentUser, sendLike } = useApp(onNavigate);
  ...
  const specialties = ['TODOS', 'Projeciologia', 'Assistenciologia', 'Mentalsomática', 'Cosmoeticologia'];

  useEffect(() => {
    if (!profiles || !currentUser) return;

    let result = profiles.filter(p => p.id !== currentUser.id);
    ...
    // Sort: Sponsored first, then Premium, then others
    result.sort((a, b) => {
  ...
  }, [profiles, currentUser, searchTerm, selectedSpecialty]);

  const handleLikeQuick = async (profileId: string) => {
    await sendLike(profileId, false);
    alert('Curtida enviada com sucesso! Se houver afinidade mútua, vocês terão um Metch.');
  };
  ...
  return (
    <div className="w-full min-h-screen bg-neutral-950 text-white pb-24 md:pb-8 pt-24 relative overflow-x-hidden">
        ...
        <div className="pointer-events-auto bg-[#08080A]/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-2 flex items-center gap-2 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] max-w-[95vw] lg:max-w-[850px] w-full relative overflow-hidden">
          ...
          <nav className="flex-1 bg-black/40 rounded-[2rem] border border-white/10 h-12 flex items-center justify-start overflow-x-auto no-scrollbar px-2 mx-1 lg:mx-2">
            <div className="flex items-center gap-1.5 w-max">
              ...
              <button onClick={() => onNavigate('grupo')} className="px-4 py-1.5 rounded-full text-xs font-bold bg-white/10 text-white border border-white/10 transition">GRUPO</button>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-6 relative z-10">
        ...
        {/* TOP SEARCH BAR & SPECIALTY TABS */}
        <div className="flex flex-col md:flex-row gap-4 mb-10 items-center justify-between border-b border-white/5 pb-6">
          <div className="relative w-full md:max-w-md">
            <Search className="absolute left-4 top-3.5 text-neutral-500" size={16} />
            <input
              type="text"
              placeholder="Buscar pesquisador por nome ou cidade..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-4 py-3 bg-[#0A0A0C] border border-white/10 rounded-2xl focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 text-sm outline-none transition text-white"
            />
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full md:w-auto pb-1">
            {specialties.map((spec) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold tracking-wider uppercase transition-all shrink-0 border ${
        ...
        {filteredProfiles.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 justify-items-center">
            {filteredProfiles.map((profile) => (
              <div
                ...
                className="bg-neutral-900/50 border border-white/5 rounded-[32px] overflow-hidden shadow-lg group relative w-full max-w-sm h-[480px] flex flex-col justify-end transition hover:border-indigo-500/30 cursor-pointer hover:shadow-[0_0_20px_rgba(99,102,241,0.1)]"
              >
                <img
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between items-center">
                  <div className="flex gap-1.5">
                    {profile.specialty && (
                      <span className="px-2.5 py-1 bg-indigo-600/80 border border-indigo-400/30 backdrop-blur-md text-[9px] font-bold text-white rounded-full uppercase tracking-wider">
                        {profile.specialty}
                      </span>
                    )}
                  </div>
                ...
                <div className="p-6 relative z-10 text-white flex flex-col">
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-baseline gap-2">
                      <span className="font-extrabold text-2xl tracking-tight">{profile.name}</span>
                      <span className="text-xl font-semibold text-neutral-300">{profile.age}</span>
                    </div>

                    <button
                      onClick={(e) => { e.stopPropagation(); setSelectedProfile(profile); }}
                      className="p-2 bg-indigo-500/20 hover:bg-indigo-500/30 border border-indigo-500/30 rounded-xl text-indigo-300 flex items-center gap-1 text-[10px] font-bold tracking-wide uppercase transition hover:scale-105 active:scale-95 cursor-pointer shadow-lg"
                    >
                      <Sparkles size={10} />
                      Ver Perfil
                    </button>
                  ...
                  <p className="text-neutral-300 text-xs mt-3 line-clamp-2 bg-neutral-950/30 p-2.5 rounded-xl border border-white/5 backdrop-blur-xs leading-relaxed font-medium">
                    {profile.bio || 'Sem biografia disponível ainda.'}
                  </p>

                  <button
                    onClick={(e) => { e.stopPropagation(); handleLikeQuick(profile.id); }}
                    className="mt-4 w-full py-2 bg-white/10 hover:bg-indigo-500 hover:text-white backdrop-blur-md rounded-xl text-[10px] font-bold transition flex items-center justify-center gap-1 cursor-pointer border border-white/5"
                  >
          ...
          <div className="p-16 text-center bg-[#08080A]/40 border border-white/10 rounded-[2.5rem] max-w-md mx-auto flex flex-col items-center">
            <span className="text-4xl text-gray-600 mb-4">🔍</span>
            <h4 className="text-md font-bold text-neutral-200">Nenhum pesquisador encontrado</h4>
            <p className="text-xs text-neutral-500 mt-2.5 max-w-xs leading-relaxed">Tente reajustar seus filtros de especialidade ou busca de nome.</p>
          </div>
        )}
      </div>
