...
import { UserProfile } from '../components/transloveTypes';
import Carousel from '../components/Carousel';
import ProfileDetailsModal from '../components/ProfileDetailsModal';
import { Sparkles, MapPin, Heart, Flame, CheckCircle2, X, RefreshCw, Star, Info, Globe, Menu } from 'lucide-react';

export default function MatchesScreen({ onNavigate }: { onNavigate: (screen: string) => void }) {
  const { profiles, currentUser, sendLike, desirePoints } = useApp(onNavigate);
  
  // Exclude current user from swipe deck
  const [swipeDeck, setSwipeDeck] = useState<UserProfile[]>([]);
  const [deckIndex, setDeckIndex] = useState(0);
  const [selectedProfile, setSelectedProfile] = useState<UserProfile | null>(null);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // Drag states for swiping cards with mouse or touch
  ...
  useEffect(() => {
    if (profiles && currentUser) {
      ...
      setSwipeDeck(filtered);
      setDeckIndex(0);
    }
  }, [profiles, currentUser]);

  const activeProfile = swipeDeck[deckIndex];

  const handleLike = async (isSuper: boolean) => {
    if (!activeProfile) return;
    const isMatch = await sendLike(activeProfile.id, isSuper);
    if (isMatch) {
      alert('Sincronia! Você e ' + activeProfile.name + ' tiveram um Metch!');
      onNavigate('chat');
    }
  ...
  return (
    <div className="w-full min-h-screen bg-neutral-950 text-white pb-24 md:pb-8 pt-24 relative overflow-x-hidden">
          ...
          <div className="flex flex-col h-full relative z-10 px-3 py-4">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-6 pb-6">
              ...
              <div
                ...
                className="relative w-full h-32 rounded-[2rem] overflow-hidden group border border-white/10 shadow-lg bg-[#121214] cursor-pointer hover:border-white/30 transition duration-300 animate-deep-breathing"
              >
                <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
                  <div className="flex justify-between items-start">
                    <div>
        ...
        <div className="pointer-events-auto bg-[#08080A]/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-2 flex items-center gap-2 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] max-w-[95vw] lg:max-w-[850px] w-full relative overflow-hidden">
          ...
          <nav className="flex-1 bg-black/40 rounded-[2rem] border border-white/10 h-12 flex items-center justify-start overflow-x-auto no-scrollbar px-2 mx-1 lg:mx-2">
            <div className="flex items-center gap-1.5 w-max px-2">
              ...
              <button onClick={() => onNavigate('matches')} className="px-4 py-1.5 rounded-full text-xs font-bold bg-white/10 text-white border border-white/10 transition">METCHS</button>
              <button onClick={() => onNavigate('chat')} className="px-3 py-1.5 rounded-full text-xs font-bold text-gray-400 hover:text-white transition">CHAT</button>
            </div>
          </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-12 relative z-10">
        ...
        {/* DESKTOP HORIZONTAL LAYOUT CONTAINER */}
        <div className="w-full flex flex-col lg:flex-row gap-10 items-center lg:items-start justify-center">
          
          {/* LEFT SIDE: Carousel & Promo Panel (Horizontal Layout on desktop) */}
          <div className="w-full lg:w-[350px] flex flex-col gap-6">
            <div className="p-6 bg-[#08080A]/60 border border-white/5 rounded-3xl backdrop-blur-md">
              <h4 className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-2">Destaques da Comunidade</h4>
              <p className="text-xs text-neutral-400 leading-relaxed mb-4">
                Encontre outros intelectuais e pesquisadores para desenvolver projetos de pesquisa interassistenciais.
              </p>
              {swipeDeck.length > 0 && (
                <div className="w-full">
                  <Carousel
                    ...
                    onLike={async (profile, isSuper) => await sendLike(profile.id, isSuper)}
                  />
                </div>
              )}
            </div>

            <div className="p-6 bg-[#08080A]/30 border border-white/5 rounded-3xl backdrop-blur-md">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-2">Seus Recursos</span>
              <div className="flex justify-between items-center">
                <span className="text-xs font-medium text-neutral-300">Afinidades Mútuas:</span>
                <span className="px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 rounded-lg text-xs font-bold">Ativas</span>
              </div>
            </div>
          </div>

          {/* RIGHT SIDE: Swipe Deck Card comparison & Controls */}
          <div className="flex-1 w-full flex flex-col items-center">
            {activeProfile ? (
              <div className="w-full flex flex-col items-center">
                
                {/* Comparison Swiping Deck (supports mouse drag or finger swipe) */}
                <div 
                  className="w-full max-w-2xl bg-[#08080A]/70 border border-white/5 rounded-[32px] p-8 flex flex-col items-center shadow-2xl backdrop-blur-md relative overflow-hidden select-none"
                  style={{
                    transform: isDragging ? `translateX(${dragOffset}px) rotate(${dragOffset * 0.03}deg)` : 'none',
                ...
                >
                  ...
                  <div className="flex items-center justify-between w-full relative gap-6 py-2">
                    
                    {/* Active profile card */}
                    <div 
                      onClick={() => setSelectedProfile(activeProfile)}
                      className="flex-1 aspect-[3/4] bg-neutral-950 rounded-3xl border border-white/10 p-1 flex flex-col justify-between overflow-hidden shadow-lg hover:border-indigo-500/30 transition duration-300 cursor-pointer group"
                    >
                      <div className="relative w-full h-[75%] rounded-2xl overflow-hidden pointer-events-none">
                        <img 
                        ...
                        {activeProfile.is_verified && (
                          <div className="absolute top-2.5 right-2.5 bg-indigo-600 p-1 rounded-full shadow-lg">
                            <CheckCircle2 size={12} fill="white" className="text-indigo-400" />
                          </div>
                        )}
                        <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1 bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg">
                          <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
                          <span className="text-[8px] font-black uppercase text-indigo-300 tracking-widest">AFINIDADE</span>
                        </div>
                      </div>
                      <div className="p-3 flex-1 flex flex-col justify-center min-w-0">
                        <span className="text-sm font-black text-white block truncate flex items-center gap-1">
                          {activeProfile.name}
                        </span>
                        <span className="text-[9px] text-indigo-400 font-extrabold uppercase tracking-widest mt-0.5 block truncate">
                    ...
                    <div className="relative z-10 flex items-center justify-center shrink-0">
                      <div className="w-16 h-16 rounded-full bg-neutral-950/90 border border-white/5 flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.3)] animate-pulse relative group">
                        <Heart 
                          size={26} 
                          fill="#6366f1" 
                          className="text-indigo-500 filter drop-shadow-[0_0_8px_rgba(99,102,241,0.6)] group-hover:scale-120 transition duration-300" 
                        />
                      </div>
                    </div>
                    ...
                    >
                      <div className="relative w-full h-[75%] rounded-2xl overflow-hidden pointer-events-none">
                        <img 
                          src={currentUser?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=300'} 
                          alt="Você" 
                          className="w-full h-full object-cover group-hover:scale-105 transition duration-500"
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent" />
                      ...
                      <div className="p-3 flex-1 flex flex-col justify-center min-w-0">
                        <span className="text-sm font-black text-white block truncate">Você</span>
                        <span className="text-[9px] text-neutral-500 font-bold uppercase tracking-wide mt-0.5 block truncate font-mono">
                          {currentUser?.city || 'Cognópolis, Foz'}
                        </span>
                      </div>
                    </div>
                  ...
                  <p className="text-[9px] font-bold text-neutral-500 uppercase tracking-widest text-center mt-6 leading-relaxed">
                    Clique nos cards para ver detalhes • Arraste com o mouse para a direita (curtir) ou esquerda (pular)
                  </p>
                </div>
                ...
                <div className="flex items-center justify-center gap-6 mt-8 pb-2">
                  <button
                    onClick={handlePass}
                    className="w-14 h-14 rounded-full bg-neutral-900 border border-white/5 flex items-center justify-center text-red-500 hover:text-red-400 hover:bg-neutral-800 hover:scale-110 transition shadow-lg cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-18 h-18 rounded-full bg-gradient-to-tr from-indigo-500 to-indigo-600 flex items-center justify-center text-white hover:scale-110 transition shadow-[0_0_25px_rgba(99,102,241,0.4)] cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-14 h-14 rounded-full bg-neutral-900 border border-white/5 flex items-center justify-center text-violet-400 hover:text-violet-300 hover:bg-neutral-800 hover:scale-110 transition shadow-lg cursor-pointer"
                  >
              ...
              <div className="w-full max-w-md p-8 bg-neutral-900/40 border border-white/10 rounded-[32px] text-center flex flex-col items-center justify-center shadow-2xl backdrop-blur-md">
                <div className="w-16 h-16 rounded-3xl bg-[#6366f1]/10 text-indigo-500 border border-indigo-500/20 flex items-center justify-center mb-6 shadow-lg shadow-indigo-500/5 animate-bounce">
                  <Sparkles size={28} />
                </div>
                <h3 className="text-lg font-extrabold tracking-tight">Metchs Esgotados!</h3>
                <p className="text-neutral-500 text-xs mt-2.5 leading-relaxed">
                  Você já avaliou todos os perfis de pesquisadores disponíveis de momento.
                </p>
                <div className="mt-8 flex flex-col gap-2.5 w-full">
                  <button
                    onClick={() => onNavigate('grupo')}
                    className="w-full py-3 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 rounded-xl font-bold text-xs tracking-wider uppercase transition shadow-lg cursor-pointer"
                  >
                    Ir para o Diretório Grupo
                  </button>
                  <button
                    onClick={handleResetDeck}
                    className="w-full py-2.5 bg-neutral-950 hover:bg-neutral-900 border border-white/10 rounded-xl text-neutral-400 hover:text-white font-bold text-xs transition flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <RefreshCw size={14} />
                    Reiniciar Deck de Perfis
                  </button>
                </div>
              </div>
            )}
